"""常駐スケジューラ。cron設定に従いジョブを定期実行する。

実行: python -m scripts.scheduler
Dockerでは独立サービス（1インスタンス）として起動する想定。
複数インスタンスで動かすと二重実行になるため、必ず1つだけ動かすこと。
"""
...
def _timezone():
    try:
        ...
        log.warning("timezone %s を解決できず既定(UTC)にフォールバック", settings.scheduler_timezone)
        return None


def main() -> None:
    if settings.auto_create_tables:
        init_db()  # 開発時の利便。本番(Alembic運用)では schema は既に存在
