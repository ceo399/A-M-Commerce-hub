...
# sku, ??, ????, ????, ????, [own_avail, own_reserved, fba_fulfillable, fba_inbound, fba_reserved]
PRODUCTS = [
    ("HAMA-USB-C-1M",   "hama USB-C ?????? 1m",  "hama",    "hama",    1280, [120, 10, 80, 20, 5]),
    ("HAMA-HDMI-2M",    "hama HDMI 2.1 ???? 2m",   "hama",    "hama",    2480, [60,  5,  40, 0,  3]),
    ("DURABLE-TRAY-A4", "DURABLE ?????? A4",      "DURABLE", "DURABLE", 1980, [200, 0,  150,30, 0]),
    ("DURABLE-BADGE",   "DURABLE ?????? 50?",    "DURABLE", "DURABLE", 3200, [90,  12, 60, 0,  4]),
    ("CORDIAL-XLR-3M",  "Cordial XLR??????? 3m", "Cordial", "Cordial", 2200, [45,  8,  30, 10, 2]),
    ("CORDIAL-TRS-5M",  "Cordial TRS???? 5m",       "Cordial", "Cordial", 3400, [25,  3,  18, 0,  1]),
    ("HAMA-MOUSE-W",    "hama ????????",        "hama",    "hama",    1680, [75,  6,  50, 15, 2]),
    ("DURABLE-STAND",   "DURABLE ????????",     "DURABLE", "DURABLE", 5600, [30,  2,  22, 5,  1]),
]

# amazon_order_id, ??(AFN/MFN), ?????, [(sku, ??, ??)], ???
SELLER_ORDERS = [
...
def main() -> None:
    ...
    with session_scope() as s:
        ...
        print("=== ??????? ?? ===")
        print(f"??: {len(prods)}? (???? {new_products}?)")
        print(f"3P??(SellerOrder): {len(orders)}? (???? {new_orders}?)")
        print(f"???????????(?????): {total_units}")
        print(f"3P?????(??): ?{order_amount:,.0f}")
